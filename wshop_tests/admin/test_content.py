...
from wshop.admin.modules.content import data
from wshop.admin.modules.content.forms import (
    BEHAVIOR_ORDER_CONFIRM_KEY, BehaviorWizardForm, CONTENT_FOOTER_KEY,
    ContentWizardForm
)
from wshop.admin.utils import wizard
from wshop.admin.views.wizard import WizardView
from wshop.notify.actions.email import SendEmail
from wshop.notify.models import Script
from wshop.notify.script import StepNext
...
def test_content_form(settings):
    ...
    settings.INSTALLED_APPS.remove("wshop.simple_cms")
    form = ContentWizardForm(shop=shop)
    ...
    settings.INSTALLED_APPS.remove("wshop.xtheme")
    form = ContentWizardForm(shop=shop)
    assert "about_us" not in form.fields
    assert "privacy_policy" not in form.fields
    assert "terms_conditions" not in form.fields
    ...
    settings.INSTALLED_APPS.append("wshop.simple_cms")
    settings.INSTALLED_APPS.append("wshop.xtheme")
...
def test_content_wizard_pane(rf, admin_user, settings):
    ...
    settings.INSTALLED_APPS.remove("wshop.simple_cms")
    settings.INSTALLED_APPS.remove("wshop.xtheme")
    settings.INSTALLED_APPS.remove("wshop.notify")
    ...
    response = WizardView.as_view()(request)
    assert response.status_code == 302
    assert response["Location"].startswith(reverse("wshop:login"))
    ...
    settings.INSTALLED_APPS.append("wshop.notify")
    assert len(wizard.load_setup_wizard_panes(shop, visible_only=False)) == 1
    request = apply_request_middleware(rf.post("/", data=pane_data), user=admin_user)
    response = WizardView.as_view()(request)
    assert response.status_code == 200
