...
from wshop.testing.factories import create_product, get_default_shop
...
def test_ajax_select_view_with_products(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "wshop.Product", "some str", admin_user)
    ...
    results = _get_search_results(rf, view, "wshop.Product", None, admin_user)
    assert len(results) == 0

    results = _get_search_results(rf, view, "wshop.Product", "product", admin_user)
...
def test_multi_select_with_main_products(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "wshop.Product", "test", admin_user)
    assert len(results) == Product.objects.count()

    results = _get_search_results(rf, view, "wshop.Product", "test", admin_user, "main")
    assert len(results) == 1
...
def test_multi_select_with_sellable_only_products(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "wshop.Product", "test", admin_user, "sellable_mode_only")
...
def test_multi_select_with_product_sales_unit(rf, admin_user):
    ...
    assert len(_get_search_results(rf, view, "wshop.Product", "Product", admin_user, sales_units="g")) == 1
    assert len(_get_search_results(rf, view, "wshop.Product", "Product", admin_user, sales_units="pcs")) == 1
    assert len(_get_search_results(rf, view, "wshop.Product", "Product", admin_user, sales_units="kg")) == 1
    assert len(_get_search_results(rf, view, "wshop.Product", "Product", admin_user, sales_units="oz")) == 1

    assert len(_get_search_results(rf, view, "wshop.Product", "Product", admin_user, sales_units="g,oz")) == 2
    assert len(_get_search_results(rf, view, "wshop.Product", "Product", admin_user, sales_units="g,kg,pcs")) == 3
    assert len(_get_search_results(rf, view, "wshop.Product", "Product", admin_user, sales_units="oz,pcs,g,kg")) == 4
...
def test_ajax_select_view_with_contacts(rf, contact_cls, admin_user):
    shop = get_default_shop()
    view = MultiselectAjaxView.as_view()
    model_name = "wshop.%s" % contact_cls._meta.model_name
...
def test_ajax_select_view_with_categories(rf, admin_user):
    ...
    results = _get_search_results(rf, view, "wshop.Category", "some str", admin_user)
    assert len(results) == 0
    ...
    results = _get_search_results(rf, view, "wshop.Category", category.name, admin_user)
    assert len(results) == 1
...
def test_multiselect_inactive_users_and_contacts(rf, regular_user, admin_user):
    ...
    # contact not in shop
    results = _get_search_results(rf, view, "wshop.PersonContact", "joe", admin_user)
    assert len(results) == 0
