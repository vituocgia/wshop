...
from wshop.admin.modules.categories.views import CategoryEditView
from wshop.admin.modules.shops.views import ShopEditView
from wshop.apps.provides import override_provides
from wshop.core import cache
from wshop.front.utils.sorts_and_filters import get_configuration
from wshop.testing.factories import get_default_category, get_default_shop
...
def test_sorts_and_filter_in_shop_edit(rf, admin_user):
    ...
    with override_settings(WSHOP_ENABLE_MULTIPLE_SHOPS=False):
        with override_provides("front_extend_product_list_form", DEFAULT_FORM_MODIFIERS):
            ...
            assert get_configuration(shop=shop) == settings.WSHOP_FRONT_DEFAULT_SORT_CONFIGURATION
            data = {
...
def test_sorts_and_filter_in_category_edit(rf, admin_user):
    ...
    with override_settings(WSHOP_ENABLE_MULTIPLE_SHOPS=False):
        with override_provides("front_extend_product_list_form", DEFAULT_FORM_MODIFIERS):
            ...
            assert get_configuration(category=category) == settings.WSHOP_FRONT_DEFAULT_SORT_CONFIGURATION
            data = {
