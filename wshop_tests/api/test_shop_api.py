...
from wshop.core.models import Currency, MutableAddress, Shop, ShopStatus
...
def test_shop_distance(admin_user):
    ...
    response = client.get("/api/wshop/shop/?lat={0}&lng={1}&distance={2}".format(my_position[0], my_position[1], 5),
                          content_type="application/json")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 1
    ...
    response = client.get("/api/wshop/shop/?lat={0}&lng={1}&distance={2}&ordering=-distance".format(my_position[0], my_position[1], 12),
                          content_type="application/json")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 3
    ...
    response = client.get("/api/wshop/shop/?lat={0}&lng={1}&distance={2}".format(my_position[0], my_position[1], 1),
                          content_type="application/json")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 1
