...
from wshop.customer_group_pricing.models import CgpDiscount, CgpPrice
...
def test_get_products(admin_user):
    ...
    request = get_request("/api/wshop/front/shop_products/", admin_user, person1)
...
def test_products_name_sort(admin_user):
    ...
    # ordering by -name
    request = get_request("/api/wshop/front/shop_products/?ordering=-name", admin_user)
    response = FrontShopProductViewSet.as_view({"get": "list"})(request)
    response.render()
    products_data = json.loads(response.content.decode("utf-8"))
...
def test_products_not_available_shop(admin_user):
    ...
    # fetch by category1
    request = get_request("/api/wshop/front/shop_products/?categories=%d" % category1.id, admin_user)
    response = FrontShopProductViewSet.as_view({"get": "list"})(request)
    response.render()
    products_data = json.loads(response.content.decode("utf-8"))
    assert len(products_data) == 1
    ...
    # fetch by category2
    request = get_request("/api/wshop/front/shop_products/?categories=%d" % category2.id, admin_user)
    response = FrontShopProductViewSet.as_view({"get": "list"})(request)
    response.render()
    products_data = json.loads(response.content.decode("utf-8"))
    assert len(products_data) == 1
    ...
    request = get_request("/api/wshop/front/shop_products/?categories=%d,%d" % (category1.id, category2.id), admin_user)
    response = FrontShopProductViewSet.as_view({"get": "list"})(request)
    response.render()
    products_data = json.loads(response.content.decode("utf-8"))
    assert len(products_data) == 2
...
def test_product_package(admin_user):
    ...
    request = get_request("/api/wshop/front/shop_products/", admin_user)
    response = FrontShopProductViewSet.as_view({"get": "list"})(request)
    response.render()
    products_data = json.loads(response.content.decode("utf-8"))
...
def test_get_newest_products(admin_user):
    ...
    response = client.get("/api/wshop/front/shop_products/?ordering=newest")
    assert response.status_code == status.HTTP_200_OK
    products = json.loads(response.content.decode("utf-8"))
    ...
    request = get_request("/api/wshop/front/shop_products/", admin_user, customer, data=data)
    response = FrontShopProductViewSet.as_view({"get": "list"})(request)
    response.render()
    assert response.status_code == status.HTTP_200_OK
    ...
    request = get_request("/api/wshop/front/shop_products/", admin_user, customer, data)
    response = FrontShopProductViewSet.as_view({"get": "list"})(request)
    response.render()
    assert response.status_code == status.HTTP_200_OK
    products = json.loads(response.content.decode("utf-8"))["results"]
...
def test_nearby_products(admin_user):
    ...
    request = get_request("/api/wshop/front/products/", admin_user, data=params)
...
def test_get_front_products(admin_user):
    ...
    request = get_request("/api/wshop/front/products/", admin_user)
    response = FrontProductViewSet.as_view({"get": "list"})(request)
    response.render()
    assert response.status_code == status.HTTP_200_OK
    products_data = json.loads(response.content.decode("utf-8"))
...
def test_product_price_info(admin_user, prices_include_tax, product_price, discount, tax_rate, taxful_price, taxless_price):
    ...
    client = _get_client(admin_user)
    response = client.get("/api/wshop/front/shop_products/", format="json")
