...
from wshop.core.models import Order, OrderStatusManager, StockBehavior
from wshop.simple_supplier.module import SimpleSupplierModule
from wshop.testing import factories
from wshop.testing.basket_helpers import get_client, REQUIRED_SETTINGS
from wshop.utils.numbers import bankers_round
...
def test_basket_with_package_product(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post("/api/wshop/basket/new/", format="json", data={"shop": shop.pk})
        ...
        for product in [sand_bag_10kg_product, sand_bag_1845kg_product, sand_bag_25kg_product]:
            response = client.post("/api/wshop/basket/{}/add/".format(basket_uuid), format="json", data={
                "shop": shop.pk,
                "product": product.id
            })
        ...
        # create order anyway
        response = client.post("/api/wshop/basket/{}/create_order/".format(basket_uuid), format="json")
        assert response.status_code == status.HTTP_201_CREATED
