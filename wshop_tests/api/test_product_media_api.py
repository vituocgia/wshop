...
from wshop.core.models import ProductMedia, ProductMediaKind
...
def test_send_product_media_image(admin_user):
    ...
    response = client.post("/api/wshop/product/%d/add_media/" % product.pk,
...
def test_product_media_api(admin_user):
    ...
    # get product media
    response = client.get("/api/wshop/product_media/%d/" % media.pk)
    ...
    # get external media
    response = client.get("/api/wshop/product_media/%d/" % media_external.pk)
    media_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.put("/api/wshop/product_media/%d/" % media.pk,
                          content_type="application/json",
                          data=json.dumps(media_data))
    ...
    # deletes an image
    response = client.delete("/api/wshop/product_media/%d/" % media.pk)
