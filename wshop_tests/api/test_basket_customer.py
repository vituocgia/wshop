...
def test_create_new_basket(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        # invalid shop
        response = client.post("/api/wshop/basket/new/", data={"shop": 1000})
        ...
        with override_settings(WSHOP_ENABLE_MULTIPLE_SHOPS=False):
            ...
            person.user = user
            person.save()
            response = client.get("/api/wshop/basket/{}/".format(basket_data['uuid']))
...
def test_create_order(admin_user, target_customer):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post("/api/wshop/basket/{}-{}/create_order/".format(shop.pk, basket.key), payload)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        response_data = json.loads(response.content.decode("utf-8"))
...
def test_basket_with_staff_user():
    with override_settings(**REQUIRED_SETTINGS):
        ...
        person = factories.create_random_person()
        response = client.post("/api/wshop/basket/new/", data={"shop": shop.pk, "customer": person.pk})
        ...
        response = client.post(
            "/api/wshop/basket/new/", data={"shop": shop.pk, "customer": staff_person.pk})
        assert response.status_code == status.HTTP_201_CREATED
        basket_data = json.loads(response.content.decode("utf-8"))
        ...
        response = client.post(
            "/api/wshop/basket/new/", data={"shop": shop.pk, "customer": person.pk})
        assert response.status_code == status.HTTP_201_CREATED
        basket_data = json.loads(response.content.decode("utf-8"))
        basket = Basket.objects.filter(key=basket_data["uuid"].split("-")[1]).first()
        assert basket.shop == shop
...
def test_basket_reorder_staff_user():
    with override_settings(**REQUIRED_SETTINGS):
        ...
        # create the basket
        response = client.post("/api/wshop/basket/new/", data={"shop": shop.pk, "customer": person.pk}, format="json")
        # Only stuff linked to shop can create baskets for someone else
        assert response.status_code == status.HTTP_403_FORBIDDEN

        # Can still add personal baskets
        staff_person = get_person_contact(staff_user)
        response = client.post("/api/wshop/basket/new/", data={"shop": shop.pk, "customer": staff_person.pk}, format="json")
        assert response.status_code == status.HTTP_201_CREATED
        basket_data = json.loads(response.content.decode("utf-8"))
        basket = Basket.objects.filter(key=basket_data["uuid"].split("-")[1]).first()
        assert basket.shop == shop
        ...
        response = client.post('/api/wshop/basket/{}-{}/add_from_order/'.format(shop.pk, basket.key), data={"order": order.pk}, format="json")
        assert response.status_code == status.HTTP_200_OK
        basket_data = json.loads(response.content.decode("utf-8"))
        ...
        response = client.post('/api/wshop/basket/{}-{}/create_order/'.format(shop.pk, basket.key))
        assert response.status_code == status.HTTP_201_CREATED
        response_data = json.loads(response.content.decode("utf-8"))
        ...
        response = client.post("/api/wshop/basket/new/", data={"shop": shop.pk, "customer": person2.pk}, format="json")
        assert response.status_code == status.HTTP_201_CREATED
        basket_data = json.loads(response.content.decode("utf-8"))
        basket = Basket.objects.filter(key=basket_data["uuid"].split("-")[1]).first()
        assert basket.shop == shop
