...
def get_products_by_id_sku(admin_user):
    ...
    # get by ID
    response = client.get("/api/wshop/product/?id=%d" % products[2].id)
    ...
    # get by SKU
    response = client.get("/api/wshop/product/?sku=%s" % products[1].sku)
    assert response.status_code == status.HTTP_200_OK
    product_data = json.loads(response.content.decode("utf-8"))
    assert len(product_data) == 1
...
def test_get_product_stocks(admin_user):
    ...
    response = client.get("/api/wshop/product/stocks/?sku=%s&supplier=%d" % (product1.sku, supplier1.id))
    assert response.status_code == status.HTTP_200_OK
    stock_data = sorted(json.loads(response.content.decode("utf-8")),
                        key=lambda prod: prod["product"])
    ...
    response = client.get("/api/wshop/product/stocks/?product=%d&supplier=%d" % (product2.id, supplier1.id))
    assert response.status_code == status.HTTP_200_OK
    stock_data = sorted(json.loads(response.content.decode("utf-8")),
                        key=lambda prod: prod["product"])
    assert len(stock_data) == 1
...
def test_create_product(admin_user):
    ...
    response = client.post("/api/wshop/product/", content_type="application/json", data=json.dumps(data))
...
def test_update_product(admin_user):
    ...
    client = _get_client(admin_user)
    data = _get_product_sample_data()
    response = client.put("/api/wshop/product/%d/" % product.pk, content_type="application/json", data=json.dumps(data))
    assert response.status_code == status.HTTP_200_OK
...
def test_delete_product(admin_user):
    ...
    response = client.delete("/api/wshop/product/%d/" % product.pk, content_type="application/json")
...
def test_create_shop_product(admin_user):
    ...
    response = client.post("/api/wshop/product/%d/add_shop/" % product.id,
...
def test_make_product_package(admin_user):
    ...
    response = client.post("/api/wshop/product/%d/make_package/" % product1.pk,
                           content_type="application/json",
...
def test_product_package(admin_user):
    ...
    # get the first child
    response = client.get("/api/wshop/product_package/%d/" % package_child_links[0].id)
    assert response.status_code == status.HTTP_200_OK
    ...
    # get the 2nd child
    response = client.get("/api/wshop/product_package/%d/" % package_child_links[1].id)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    # get the 3rd child
    response = client.get("/api/wshop/product_package/%d/" % package_child_links[2].id)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.put("/api/wshop/product_package/%d/" % package_child_links[0].id,
                          content_type="application/json",
                          data=json.dumps(package_data))
    assert response.status_code == status.HTTP_200_OK
    response = client.get("/api/wshop/product_package/%d/" % package_child_links[0].id)
    ...
    response = client.delete("/api/wshop/product_package/%d/" % package_child_links[2].id)
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
def test_product_attribute(admin_user):
    ...
    response = client.post("/api/wshop/product/%d/add_attribute/" % product1.pk,
                           content_type="application/json",
                           data=json.dumps(product_attr1_data))
    assert response.status_code == status.HTTP_201_CREATED
    response = client.post("/api/wshop/product/%d/add_attribute/" % product1.pk,
    ...
    # get the first attr
    response = client.get("/api/wshop/product_attribute/%d/" % attrs[0].id)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    # get the 2nd attr
    response = client.get("/api/wshop/product_attribute/%d/" % attrs[1].id)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert data["product"] == product1.id
    ...
    response = client.put("/api/wshop/product_attribute/%d/" % attrs[0].id,
                          content_type="application/json",
    ...
    response = client.delete("/api/wshop/product_attribute/%d/" % attrs[1].id)
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
def test_create_complete_product(admin_user):
    ...
    response = client.post("/api/wshop/attribute/", content_type="application/json", data=json.dumps(attribute_data))
    assert response.status_code == status.HTTP_201_CREATED
    ...
    response = client.post("/api/wshop/manufacturer/", content_type="application/json", data=json.dumps(manufac_data))
    assert response.status_code == status.HTTP_201_CREATED
    ...
    response = client.post("/api/wshop/sales_unit/", content_type="application/json", data=json.dumps(sales_unit_data))
    assert response.status_code == status.HTTP_201_CREATED
    ...
    response = client.post("/api/wshop/tax_class/", content_type="application/json", data=json.dumps(tax_class_data))
    assert response.status_code == status.HTTP_201_CREATED
    ...
    response = client.post("/api/wshop/product/", content_type="application/json", data=json.dumps(product_data))
    product = Product.objects.first()
    assert product
