...
from wshop.core.models import TaxClass
from wshop.testing.factories import get_default_shop, create_product
...
def test_tax_class_api(admin_user):
    ...
    response = client.post("/api/wshop/tax_class/",
    ...
    response = client.put("/api/wshop/tax_class/%d/" % tax_class.id,
                          content_type="application/json",
                          data=json.dumps(tax_class_data))
    ...
    response = client.get("/api/wshop/tax_class/%d/" % tax_class.id)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert tax_class.name == data["translations"]["en"]["name"]
    assert tax_class.enabled == tax_class_data["enabled"]

    response = client.get("/api/wshop/tax_class/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert tax_class.name == data[0]["translations"]["en"]["name"]
    assert tax_class.enabled == data[0]["enabled"]

    response = client.delete("/api/wshop/tax_class/%d/" % tax_class.id)
