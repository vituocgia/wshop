...
from wshop.core.models import OrderLineType, PaymentStatus, StockBehavior
...
def test_refund_errors(admin_user):
    ...
    data = {"restock_products": False}
    response = client.post("/api/wshop/order/%s/create_full_refund/" % order.id, data, format="json")
    assert response.status_code == status.HTTP_400_BAD_REQUEST
