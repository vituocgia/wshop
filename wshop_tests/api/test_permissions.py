...
from wshop.testing.factories import get_default_shop, UserFactory
...
def test_api_permissions_anonymous():
    ...
    assert client.get("/api/wshop/user/").status_code == status.HTTP_401_UNAUTHORIZED
    assert client.post("/api/wshop/user/", {"email": "xiloca@djsij.com"}).status_code == status.HTTP_401_UNAUTHORIZED

    # set API Public WRITE - access granted
    config.set(None, permission_key, PermissionLevel.PUBLIC_WRITE)
    response = client.get("/api/wshop/user/")
    ...
    # DELETE data too
    assert client.delete("/api/wshop/user/%d/" % users[-1].id).status_code == status.HTTP_204_NO_CONTENT
    users.pop()
    ...
    assert client.delete("/api/wshop/user/%d/" % users[-1].id).status_code == status.HTTP_401_UNAUTHORIZED
    ...
    assert client.get("/api/wshop/user/").status_code == status.HTTP_401_UNAUTHORIZED
    assert client.put("/api/wshop/user/", user_data[0]).status_code == status.HTTP_401_UNAUTHORIZED
...
def test_api_permissions_authenticated_user():
    ...
    # set API disabled
    config.set(None, permission_key, PermissionLevel.DISABLED)
    assert client.get("/api/wshop/user/").status_code == status.HTTP_403_FORBIDDEN
    assert client.post("/api/wshop/user/", {"email": "xiloca@djsij.com"}).status_code == status.HTTP_403_FORBIDDEN
    ...
    # DELETE data - nope, not a safe method
    assert client.delete("/api/wshop/user/%d/" % users[0].id).status_code == status.HTTP_403_FORBIDDEN
    ...
    config.set(None, permission_key, PermissionLevel.AUTHENTICATED_WRITE)
    assert client.get("/api/wshop/user/").status_code == status.HTTP_200_OK
    assert client.delete("/api/wshop/user/%d/" % users[-1].id).status_code == status.HTTP_204_NO_CONTENT
    users.pop()
