...
from wshop.core.models import Shop, ShopProduct, StockBehavior, Supplier
...
def test_get_suppliers(admin_user):
    ...
    # list suppliers
    response = client.get("/api/wshop/supplier/")
    ...
    # get supplier by id
    response = client.get("/api/wshop/supplier/%s/" % supplier2.pk)
    assert response.status_code == status.HTTP_200_OK
    supplier_data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.get("/api/wshop/supplier/%s/stock_statuses/" % supplier1.pk)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/wshop/supplier/%s/stock_statuses/" % supplier1.pk, format="json", data={
