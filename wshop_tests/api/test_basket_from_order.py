...
from wshop.core.models import AnonymousContact, get_person_contact
...
def test_copy_order_to_basket_for_staff(admin_user):
    with override_settings(**REQUIRED_SETTINGS):
        ...
        response = client.post(
            '/api/wshop/basket/%s/set_customer/' % uuid, format="json", data={"customer": customer.id}
        )
        assert response.status_code == status.HTTP_200_OK

        response = client.post('/api/wshop/basket/%s/add_from_order/' % uuid, {"order": order.pk})
