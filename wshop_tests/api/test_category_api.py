...
def test_category_api(admin_user):
    ...
    response = client.post("/api/wshop/category/",
    ...
    response = client.put("/api/wshop/category/%d/" % category.id,
                          content_type="application/json",
                          data=json.dumps(category_data))
    ...
    response = client.get("/api/wshop/category/%d/" % category.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/wshop/category/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert category.name == data[0]["translations"]["en"]["name"]
    assert category.ordering == data[0]["ordering"]

    response = client.get("/api/wshop/category/?status=%s" % CategoryStatus.VISIBLE.value)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 1

    response = client.get("/api/wshop/category/?status=%s" % CategoryStatus.INVISIBLE.value)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 0

    response = client.get("/api/wshop/category/?status=%s" % CategoryStatus.DELETED.value)
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    assert len(data) == 0

    # soft delete
    response = client.delete("/api/wshop/category/%d/" % category.id)
