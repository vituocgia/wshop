...
from wshop.core.models import Attribute, AttributeType, AttributeVisibility
...
def test_attr_api(admin_user):
    ...
    response = client.post("/api/wshop/attribute/",
    ...
    response = client.put("/api/wshop/attribute/%d/" % attr.id,
                          content_type="application/json",
                          data=json.dumps(attr_data))
    ...
    response = client.get("/api/wshop/attribute/%d/" % attr.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.get("/api/wshop/attribute/")
    assert response.status_code == status.HTTP_200_OK
    data = json.loads(response.content.decode("utf-8"))
    ...
    response = client.delete("/api/wshop/attribute/%d/" % attr.id)
