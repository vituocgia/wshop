...
from wshop.core.models import MutableAddress
from wshop.testing.factories import create_random_person, get_default_shop
...
def test_mutable_address_api(admin_user):
    ...
    response = client.post("/api/wshop/address/",
    ...
    response = client.put("/api/wshop/address/%d/" % mutable_address.id,
                          content_type="application/json",
                          data=json.dumps(mutable_address_data))
    ...
    response = client.get("/api/wshop/address/%d/" % mutable_address.id)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.patch("/api/wshop/address/%d/" % mutable_address.id,
                            content_type="application/json",
                            data=json.dumps(mutable_address_data))
    assert response.status_code == status.HTTP_200_OK
    mutable_address = MutableAddress.objects.first()
    ...
    response = client.delete("/api/wshop/address/%d/" % mutable_address.id)
...
def test_mutable_address_field_properties_settings_api(admin_user, settings):
    settings.WSHOP_ADDRESS_FIELD_PROPERTIES["postal_code"] = {"required": True}
    settings.WSHOP_ADDRESS_FIELD_PROPERTIES["street2"] = {"required": True}
    ...
    del(settings.WSHOP_ADDRESS_FIELD_PROPERTIES["postal_code"])
    del(settings.WSHOP_ADDRESS_FIELD_PROPERTIES["street2"])
