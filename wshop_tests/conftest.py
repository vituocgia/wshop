...
def pytest_configure(config):
    setting_changed.connect(clear_caches, dispatch_uid="wshop_test_clear_caches")
    settings.WSHOP_TELEMETRY_ENABLED = False


def pytest_runtest_call(item):
    # All tests are run with a theme override `wshop.themes.classic_gray.ClassicGrayTheme`.
    # To un-override, use `with override_current_theme_class()` (no arguments to re-enable database lookup)
    from wshop.themes.classic_gray.theme import ClassicGrayTheme
