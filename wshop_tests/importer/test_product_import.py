...
from wshop.core.models import Category, Manufacturer, Product, ShopProduct
from wshop.default_importer.importers import ProductImporter
...
def test_sample_import_no_match(stock_managed):
    ...
    importer.manually_match("gtiin", "wshop.core.models.Product:gtin")
    importer.do_remap()
    assert len(importer.unmatched_fields) == 0
    importer.do_import(ImportMode.CREATE_UPDATE)
    products = importer.new_objects
...
def test_strange_category_name():
    import_categoryfile(
        ...
        map_from="mycat",
        map_to="wshop.core.models.ShopProduct:categories"
    )
