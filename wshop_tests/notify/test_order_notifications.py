...
from wshop.core.order_creator import OrderCreator
...
from wshop.testing.mock_population import populate_if_required
from wshop.testing.soup_utils import extract_form_fields
from wshop_tests.admin.test_order_creator import (
    get_frontend_order_state, get_order_from_state
)
from wshop_tests.front.test_checkout_flow import (
    _populate_client_basket, fill_address_inputs
)
from wshop_tests.functional.test_refunds import (
...
from wshop_tests.utils.fixtures import (
    ...
    for key, value in extract_form_fields(soup.find('form', id='addresses')).items():
        if not value:
            ...
            if not value and "email" in key:
                value = "test%d@example.wshop.com" % random.random()
            if not value:
                value = "test"
        inputs[key] = value
...
def test_order_received_variables(rf, with_shop_contact):
    ...
    if with_shop_contact:
        STEP_DATA[0]['actions'].insert(0, {
            ...
            "recipient": {
                "constant": "someoneelse@wshop.com"
            }
        })
