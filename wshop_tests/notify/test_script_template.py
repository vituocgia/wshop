...
from wshop.notify.admin_module.views import (
    ScriptTemplateConfigView, ScriptTemplateEditView, ScriptTemplateView
)
from wshop.notify.models import Script
from wshop.simple_supplier.notify_events import AlertLimitReached
...
def test_generic_script_template_admin(rf, admin_user, script_template_cls):
    ...
    assert response.url == reverse("wshop_admin:notify.script-template-config", kwargs={"id": identifier})
    ...
    assert response.status_code == 302
    assert response.url == reverse("wshop_admin:notify.script.list")
...
def test_stock_alert_limit_script_template_manual(rf):
    ...
    data = {
        "en-body": "my body",
        "en-subject": "something",
        "base-recipient": "someemail@wshop.com",
        "base-last24hrs": True
    }
    form = script_template.get_form(data=data)
    assert form.is_valid()
    ...
    data.update({
        "en-body": "my body 2",
        "en-subject": "something 2",
        "base-recipient": "someemail@wshop.comzzz",
    })
    form = script_template.get_form(data=data)
    assert form.is_valid()
    edited_script = script_template.update_script(form)
