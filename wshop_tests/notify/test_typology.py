...
from wshop.notify.base import Binding
from wshop.notify.enums import StepConditionOperator
from wshop.notify.script import Context
from wshop.notify.typology import Enum, Language, Model, Text
...
def test_model_type_matching():
    assert empty_iterable(Binding("x", type=Model("wshop.Contact")).get_matching_types(ATestEvent.variables))
    assert Binding("x", type=Model("wshop.Order")).get_matching_types(ATestEvent.variables) == {"order"}
