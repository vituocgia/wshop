...
from wshop.admin.modules.orders.views.addresses import ADDRESS_EDITED_LOG_IDENTIFIER
...
from wshop.testing.factories import create_empty_order, get_default_shop
...
def change_addresses(live_server, browser, order):
    edit_url = reverse("wshop_admin:order.edit-addresses", kwargs={"pk": order.pk})
...
def set_status(browser, order, status):
    ...
    form_action = reverse("wshop_admin:order.set-status", kwargs={"pk": order.pk})
