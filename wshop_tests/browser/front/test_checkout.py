...
from wshop.core.models import OrderStatus, OrderStatusRole
...
from wshop.utils.importing import clear_load_cache
...
@pytest.mark.urls('wshop.testing.single_page_checkout_test_urls')
@pytest.mark.browser
@pytest.mark.djangodb
def test_browser_checkout_vertical(browser, live_server, settings): 
    with override_settings(WSHOP_CHECKOUT_VIEW_SPEC=("wshop.front.views.checkout:SinglePageCheckoutView")):
        # initialize
        product_name = "Test Product"
        get_default_shop()
        pm = get_default_payment_method()
