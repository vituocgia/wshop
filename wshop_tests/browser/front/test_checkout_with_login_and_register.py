...
@override_settings(WSHOP_REGISTRATION_REQUIRES_ACTIVATION=False)
@pytest.mark.urls('wshop.testing.checkout_with_login_and_register_urls')
@pytest.mark.browser
...
@override_settings(WSHOP_REGISTRATION_REQUIRES_ACTIVATION=False)
@pytest.mark.urls('wshop.testing.single_page_checkout_with_login_and_register_conf')
@pytest.mark.browser
@pytest.mark.djangodb
...
def register_test(browser, live_server, test_username, test_email, test_password):
    ...
    product_url = reverse("wshop:product", kwargs={"pk": product.pk, "slug": product.slug})
