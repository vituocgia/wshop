...
from wshop.testing.browser_utils import click_element, wait_until_condition
...
def test_category_product_filters_1(browser, live_server, settings):
    ...
    url = reverse("wshop:category", kwargs={"pk": first_cat.pk, "slug": first_cat.slug})
...
def second_category_page_change(browser, live_server, shop, category):
    url = reverse("wshop:category", kwargs={"pk": category.pk, "slug": category.slug})
    browser.visit("%s%s" % (live_server, url))
