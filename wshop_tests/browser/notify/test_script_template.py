...
from wshop.front.apps.registration.notify_events import (
    RegistrationReceivedEmailScriptTemplate
)
from wshop.front.notify_script_templates.generics import (
...
from wshop.simple_supplier.notify_script_template import (
    StockLimitEmailScriptTemplate
)
from wshop.testing.browser_utils import (
    click_element, wait_until_condition
)
from wshop.testing.notify_script_templates import DummyScriptTemplate
from wshop.testing.utils import initialize_admin_browser_test

pytestmark = pytest.mark.skipif(os.environ.get("WSHOP_BROWSER_TESTS", "0") != "1", reason="No browser tests run.")
...
def test_generic_custom_email_script_template(browser, admin_user, live_server, settings, script_template_cls):
    ...
    browser.find_by_id("id_base-recipient").fill("other@wshop.com")
    browser.find_by_css("form button.btn.btn-lg.btn-primary").first.click()
    ...
    assert len(serialized_steps[0]["actions"]) == 1
    assert len(serialized_steps[0]["conditions"]) == 0
    assert serialized_steps[0]["actions"][0]["recipient"]["constant"] == "other@wshop.com"
...
def test_stock_alert_limit_script_template(browser, admin_user, live_server, settings):
    ...
    # fill form
    subject = "changed sub"
    recipient = "changed.email@wshop.com"
    browser.find_by_id("id_en-subject").fill(subject)
    browser.find_by_id("id_base-recipient").fill(recipient)
