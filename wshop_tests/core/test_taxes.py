...
from wshop.core.models import CustomerTaxGroup, OrderLineType
from wshop.core.order_creator import OrderCreator, TaxesNotCalculated
...
from wshop_tests.campaigns.test_reports import seed_source
...
def test_calculate_taxes_automatically_setting():
    with override_settings(WSHOP_CALCULATE_TAXES_AUTOMATICALLY_IF_POSSIBLE=True):
        source = get_source()
    ...
    with override_settings(WSHOP_CALCULATE_TAXES_AUTOMATICALLY_IF_POSSIBLE=False):
        source = get_source()
        source.get_final_lines()
        assert source._taxes_calculated == False
