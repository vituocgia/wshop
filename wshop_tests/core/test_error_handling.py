...
from wshop.admin.error_handlers import AdminPageErrorHandler
from wshop.admin.views.dashboard import DashboardView
from wshop.core.error_handling import install_error_handlers
from wshop.front.error_handlers import FrontPageErrorHandler
from wshop.front.views.index import IndexView
from wshop_tests.utils import replace_urls


def setup_function(fn):
    # clear the cache for error handlers
    import wshop.core.error_handling as error_handling
...
def test_front_error_handlers(rf):
    """
    Test that WSHOP_ERROR_PAGE_HANDLERS_SPEC installs error handlers overwriting custom ones
    """
    with override_settings(
        DEBUG=False,
        WSHOP_ERROR_PAGE_HANDLERS_SPEC=["wshop.front.error_handlers.FrontPageErrorHandler"],
        MIDDLEWARE_CLASSES=[],
...
def test_admin_error_handlers(rf):
    ...
    with override_settings(
        DEBUG=False,
        WSHOP_ERROR_PAGE_HANDLERS_SPEC=["wshop.admin.error_handlers.AdminPageErrorHandler"],
        MIDDLEWARE_CLASSES=[],
        TEMPLATES=[  # Overriden to be sure about the contents of our 500.jinja
            {
                "BACKEND": "django_jinja.backend.Jinja2",
...
def test_install_error_handlers(rf):
    ...
    with override_settings(DEBUG=False, WSHOP_ERROR_PAGE_HANDLERS_SPEC=[], MIDDLEWARE_CLASSES=[]):
