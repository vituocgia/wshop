...
from wshop.core.models._order_utils import (
...
def test_refno_generation(method):
    for attempt in range(10):
        with override_settings(WSHOP_REFERENCE_NUMBER_METHOD=method):
...
def test_custom_ident_generation():
    ...
    for method in methods:
        with override_settings(WSHOP_ORDER_IDENTIFIER_METHOD=method):
            order = create_empty_order()
            order.save()
