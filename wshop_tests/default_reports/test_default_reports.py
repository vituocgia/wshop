...
from wshop.default_reports.reports import (
...
from wshop.reports.admin_module.views import ReportView
from wshop.reports.forms import DateRangeChoices
from wshop.reports.writer import get_writer_instance
...
from wshop_tests.core.test_basic_order import create_order
from wshop_tests.reports.test_reports import initialize_report_test
...
def test_product_total_sales_report(rf, admin_user, order_by):
    with override_provides("reports", ["wshop.default_reports.reports.product_total_sales:ProductSalesReport"]):
        shop = get_default_shop()
        supplier = get_default_supplier()
...
def test_new_customers_report(rf, admin_user, group_by):
    with override_provides("reports", ["wshop.default_reports.reports.new_customers:NewCustomersReport"]):
        shop = get_default_shop()
