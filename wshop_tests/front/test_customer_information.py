...
from wshop.front.apps.customer_information.forms import PersonContactForm
...
def default_customer_data():
    return {
        ...
        "contact-last_name": "Wshop",
        "contact-email": "captain@wshop.local",
        "contact-gender": "o",
    }
...
def default_address_data(address_type):
    return {
        ...
        "{}-email".format(address_type): "captain@wshop.local",
        "{}-street".format(address_type): "123 Fake St.",
        "{}-postal_code".format(address_type): "1234567",
        "{}-city".format(address_type): "Wshopville",
...
def test_new_user_information_edit():
    ...
    new_email = "nyyppa@example.wshop.com"
...
def test_company_tax_number_limitations(regular_user, allow_company_registration):
    ...
    if allow_company_registration:
        ...
        user = User.objects.create_user(new_user_username, "derpy@wshop.com", new_user_password)
...
def test_person_contact_form_field_overrides():
    with override_settings(WSHOP_PERSON_CONTACT_FIELD_PROPERTIES={}):
