...
from wshop.core.models import get_person_contact
from wshop.core.utils.forms import MutableAddressForm
from wshop.front.checkout.addresses import AddressesPhase
from wshop.front.checkout.confirm import ConfirmForm
from wshop.front.views.checkout import BaseCheckoutView
...
@override_settings(WSHOP_ADDRESS_HOME_COUNTRY="FI")
...
def test_required_address_fields():
    with override_settings(WSHOP_ADDRESS_FIELD_PROPERTIES={}):
        form = MutableAddressForm()
...
class AddressesOnlyCheckoutView(BaseCheckoutView):
        phase_specs = ['wshop.front.checkout.addresses:AddressesPhase']
...
def test_confirm_form_field_overrides():
    with override_settings(WSHOP_CHECKOUT_CONFIRM_FORM_PROPERTIES={}):
        form = ConfirmForm()
