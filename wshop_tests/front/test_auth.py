...
from wshop_tests.utils.fixtures import regular_user, REGULAR_USER_PASSWORD
...
def test_login_logs_the_user_in(client, regular_user, rf):
    if "wshop.front.apps.auth" not in settings.INSTALLED_APPS:
        pytest.skip("Need wshop.front.apps.auth in INSTALLED_APPS")

    get_default_shop()
    prepare_user(regular_user)
    redirect_target = "/redirect-success/"
    response = client.post(reverse("wshop:login"), data={
...
def test_login_fails_without_valid_password(client, regular_user, rf):
    ...
    prepare_user(regular_user)
    get_default_shop()
    client.post(reverse("wshop:login"), data={
        "username": regular_user.username,
