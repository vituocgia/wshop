...
from wshop.admin.modules.contacts.views import ContactDetailView
from wshop.core.models import CompanyContact
from wshop.core.models import PersonContact
from wshop.front.apps.customer_information.views import CompanyEditView
...
def test_registration(django_user_model, client, requiring_activation):
    ...
    with override_settings(
        WSHOP_REGISTRATION_REQUIRES_ACTIVATION=requiring_activation,
    ):
...
def test_registration_2(django_user_model, client, requiring_activation):
    ...
    with override_settings(
        ...
        response = client.post(reverse("wshop:registration_register"), data={
            "username": username,
            "email": email,
            "password1": "password",
            "password2": "password",
            "next": reverse('wshop:checkout')
        })
        user = django_user_model.objects.get(username=username)
        assert response.status_code == 302 #redirect
        assert response.url.endswith(reverse('wshop:checkout'))
...
def test_password_recovery_user_receives_email_1(client):
    ...
    user = get_user_model().objects.create_user(
        ...
        password="asdfg",
        email="random@wshop.local"
    )
    n_outbox_pre = len(mail.outbox)
    client.post(
        reverse("wshop:recover_password"),
        data={
            "email": user.email
        }
    )
...
def test_password_recovery_user_receives_email_3(client):
    ...
    n_outbox_pre = len(mail.outbox)
    # Recover all users with email random@wshop.local
    client.post(
        reverse("wshop:recover_password"),
        data={
            "email": user.email
...
def test_user_will_be_redirected_to_user_account_page_after_activation(client, requiring_activation):
    ...
    if "wshop.front.apps.customer_information" not in settings.INSTALLED_APPS:
        pytest.skip("wshop.front.apps.customer_information required in installed apps")
    ...
    with override_settings(
        ...
        if requiring_activation:
            ...
            assert reverse('wshop:customer_edit') == response.request['PATH_INFO'], 'user should be on the account-page.'
        else:
            assert len(mail.outbox) == 0
            assert user.is_active
            assert reverse('wshop:index') == response.request['PATH_INFO']
