...
- Add ``WshopMiddleware`` to set the current request shop
...
- API: ``wshop.front.apps.auth.forms.PasswordRecoveryForm`` to core
...
- Add WSHOP_CHECKOUT_CONFIRM_FORM_PROPERTIES setting which can be used
...
- Fix default OrderStatus identifiers and add a management command
  ``wshop_fix_order_status_identifiers`` to fix them in the database too
...
- Add WSHOP_PERSON_CONTACT_FIELD_PROPERTIES setting which can be used
  to change person contact form field properties
- Fix caching of ``wshop.product.is_visible`` template function
...
- Add Wshop version to static urls
...
- ``WSHOP_REFERENCE_NUMBER_METHOD``, ``WSHOP_REFERENCE_NUMBER_LENGTH``
  and ``WSHOP_REFERENCE_NUMBER_PREFIX`` are now mere defaults and can be
  changed from settings under main menu "Settings > Other Settings >
  System Settings".
- Changed ``WSHOP_REFERENCE_NUMBER_LENGTH`` from 10 to 17
...
- Add the setting ``WSHOP_ERROR_PAGE_HANDLERS_SPEC`` to handle custom error
...
- Add ``WSHOP_AUTO_SHOP_PRODUCT_CATEGORIES`` option that
...
- Make form modifiers reusable. Users of ``ShipmentFormModifier``
  ...
  ``wshop.admin.form_modifier.FormModifier`` interface instead
...
- Remove shop languages, category, tax class, service provider and services
  default record creation from ``wshop_init`` management command
...
- Configure category and search sorts and filters.
    ...
    - This change should be noted when updating latest
      front for projects using ``wshop.front``
...
- Use default address form from core for customer information and
   checkout address.
- Move WSHOP_FRONT_ADDRESS_FIELD_PROPERTIES to core and rename it to
   WSHOP_ADDRESS_FIELD_PROPERTIES.
...
- Prevent Wshop from loading if Parler related settings are missing
...
- Move ``wshop/order_printouts/pdf_export.py`` to ``wshop/utils/pdf.py``
...
The first Wshop release.

Content of Wshop 0.4.0 is same as :doc:`Shoop 4.0.0 <shoop-changelog>`
with all "shoop" texts replaced with "wshop".
