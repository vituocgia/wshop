...
This part of the documentation covers the structural elements of Wshop's default
...
instructions you should already have a working Wshop installation with the
default frontend theme up and running. If not, you can start by reading
:doc:`Getting Started guide <../howto/getting_started_dev>`.

Wshop's default frontend theme
------------------------------

Wshop's frontend templates are written with `Jinja2 <http://jinja.pocoo.org/>`_
...
Wshop's template files are easy to modify and basic knowledge of HTML and CSS
takes you far. Wshop's frontend and the default theme already include the necessary
...
Wshop utilizes a similar folder structure for all the templates in different apps.
All the template files are always included in the app folder ``wshop/APP/templates/``.

Within this template folder the folder structure is: ``APP/MODULE/TEMPLATE.jinja``.
For example, this could be converted into ``wshop/product/detail.jinja``

The default frontend theme can be found in ``wshop/themes/classic_gray``.
...
.. topic:: Example
   ...
   folder: ``wshop/simple_cms/templates/`` where the path to the template file
   is ``wshop/simple_cms/page.jinja``.
...
application templates under ``wshop/front/apps/``. Each app has it's own
...
purpose. Template files inherit the base layout from ``wshop/base.jinja``.


General
^^^^^^^

General template files can be found under ``wshop/front/templates/``

**Base** ``wshop/front/base.jinja``
...
**Index** ``wshop/front/index.jinja``
    Your shop's home page.

**Macros** ``wshop/front/macros.jinja``
...
Product and category templates can be found under ``wshop/front/templates/``

**Detail** ``wshop/front/product/detail.jinja``
...
**Category** ``wshop/front/product/category.jinja``
...
All shopping basket related templates go in the ``wshop/front/templates/wshop/front/basket``
...
**Default Basket** ``wshop/front/basket/default_basket.jinja``
    The structure of shopping basket. It includes the shopping basket's
    contents as a table from a separate macro in ``wshop/front/templates/wshop/front/macros/basket.jinja``.
    The ordering form macro is also being rendered in this file.

Orders
^^^^^^

Order related templates can be found in ``wshop/front/templates/wshop/front/order/``.

**Complete** ``wshop/front/order/complete.jinja``
    Displays the order success message and details of the order.

**Payment Canceled** ``wshop/front/order/payment_canceled.jinja``
...
``wshop/apps/simple_search/templates``

**Search** ``wshop/simple_search/search.jinja``
...
Authentication through the Wshop Front is another sub-app.
Its templates can be found in its own folder:
``wshop/front/apps/auth/templates/wshop/user/``
...
``wshop/front/apps/registration/templates``

**Registration Form** ``wshop/registration/register.jinja``
    Registration form template for new users.

**Activation Failed** ``wshop/registration/activation_failed.jinja``
...
Its templates can be found in:
``wshop/front/apps/customer_information/templates/``

**Edit** ``wshop/customer_information/edit.jinja``
...
``wshop/front/apps/personal_order_history/templates/``

**Order Detail** ``wshop/personal_order_history/order_detail.jinja``
    Template for displaying single order's details.

**Order List** ``wshop/personal_order_history/order_list.jinja``
...
This paragraph explains how to register template functions in Wshop's sub-apps.
...
The template helpers can be used in templates with ``wshop.<module_name>.<TemplateHelper::method>()``.
For example ``wshop.my_module.get_day_names()``.
...
The basic principle of deriving custom Wshop templates is not to modify the
...
If your own application is listed before ``wshop.front`` (and/or other theme apps)
...
.. topic:: Example
  ...
  Simply copy ``index.jinja`` from ``wshop/front/templates/wshop/index.jinja``
  to your application's template folder ``myshop/templates/wshop/index.jinja``,
  then modify it to your heart's content.

  Now let's say you want to tweak the product category view too.

  Copy ``wshop/front/templates/wshop/product/category.jinja`` to
  ``myshop/templates/wshop/product/category.jinja``, then start modifying.
