Prices and Taxes in Wshop
=========================

This document gives an overview of Wshop's pricing and tax mechanics.
...
Prices in Wshop have an unit that is combination of a currency and an
...
usually decided by the `~wshop.core.models.Shop`, which has `currency`
and `prices_include_tax` fields.  In general, it is also possible that
the active `~wshop.core.pricing.PricingModule` uses a different price
unit that is specified by the shop.  Currently there is no such pricing
module in the Wshop Base distribution.
...
The price unit of a `~wshop.core.models.Shop` can be changed as long as
there are no `Orders <wshop.core.models.Order>` created for the shop.

The price unit of an `~wshop.core.models.Order` is stored in its
...
Taxes in Wshop are implemented by a `tax module
<wshop.core.taxing.TaxModule>`.  The Wshop Base distribution ships a tax
...
`~wshop.core.settings.WSHOP_CALCULATE_TAXES_AUTOMATICALLY_IF_POSSIBLE`
setting is true, then on-demand calculation will be used.  Otherwise
on-checkout calculation will be used.

With on-checkout tax calculation mode Wshop calculates taxes for a
...
<wshop.core.models.OrderLineTax>` object linked to the order line.
These line tax objects contain references to `~wshop.core.models.Tax`
...
Wshop Default Tax is a tax module that calculates taxes based on a set
...
The tax rules of Default Tax can be managed in the Wshop Shop Admin
