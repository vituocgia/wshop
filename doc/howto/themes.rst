...
The look and feel of your Wshop storefront can be completely customized to fit your
brand using custom themes and macros.

In Wshop lingo, themes are :doc:`addons <addons>` that customize the look and
...
by Wshop base, as an example. This theme can be found in `wshop/themes/classic_gray`.
...
``theme_folder/locale``
    ...
    running ``python -m wshop_workbench wshop_makemessages -l en``.
...
``theme_folder/templates/wshop``
    Contains all templates overrides.
    **Note** that ``theme_folder/wshop/front/index.jinja`` is a required file.

``theme_folder/templates/wshop/front/macros/theme``
...
``theme_folder/apps.py``
    This file contains the ``AppConfig`` for your theme. The ``AppConfig`` is the mechanism by which Wshop recognizes
...
Most of the Wshop front templates are found in ``wshop/front/templates/wshop/front/``
and macros in ``wshop/front/templates/wshop/front/macros/``.

Additionally, Wshop has some applications for front (found in ``wshop/front/apps``).
These apps define their own templates in
in ``wshop/front/apps/<appname>/templates/``.


Templates
~~~~~~~~~

Lets walk through two typical use cases you may encounter when overriding Wshop templates.
...
.. code-block:: python
    ...
    class MyTheme(ClassicGrayTheme):
        template_dir = "mytheme"  # your templates should be in templates/mytheme/wshop/

.. note:: In a real project, you can use any ``Theme`` as the parent.

.. note:: ``templates/mytheme/wshop/front/index.jinja`` must exist for the theme to work.


**Case A**
    *Overriding a Wshop front template*
    ...
    in ``wshop/front/templates/wshop/front/product/category.jinja``.

    You can then copy said file to ``templates/mytheme/wshop/front/product/`` and make your changes.

**Case B**,
    *Overriding a Wshop front app template*
    ...
    ``wshop/front/apps/simple_search/templates/wshop/simple_search/search_form.jinja``.
...
The original macro definitions used by Wshop base theme can be
found in ``wshop/front/templates/wshop/front/macros``. Inside this folder,
...
create ``templates/mytheme/wshop/front/macros/theme/category.jinja``
...
See ``wshop/themes/classic_gray/`` for examples on how to define multiple stylesheets and
``wshop/front/templates/wshop/front/base.jinja`` for how to use them in your own `base.jinja`
...
Wshop themes support our xtheme template engine which offers custom
...
.. note::
  ...
  your theme even with very basic Wshop installation. If you are unsure,
