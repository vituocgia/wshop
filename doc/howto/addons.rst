...
Wshop contains facilities for installing, detecting, loading and configuring
...
occurs via the Wshop :doc:`Provides <../ref/provides>` subsystem.

Configuring your project to load addons
---------------------------------------

The Wshop addon manager handles adding addons into Django's ``INSTALLED_APPS``
list during project initialization time.

It's easy to convert a standard Django configuration to be addons enabled.

For instance, take a bare-bones Wshop core installation.
...
configuration key, ``WSHOP_ENABLED_ADDONS_FILE``, to name a path to a
configuration file that is writable by the application server.

The ``wshop.addons.add_enabled_addons()`` method manages reading this file,
...
.. code-block:: python
    ...
    from wshop.addons import add_enabled_addons

    # *snip*

    # This varies depending on how your particular project arranges writable files.
    WSHOP_ENABLED_ADDONS_FILE = os.path.join(BASE_DIR, "enabled_addons")

    INSTALLED_APPS = add_enabled_addons(WSHOP_ENABLED_ADDONS_FILE, (
        'django.contrib.admin',
        'django.contrib.auth',
        'django.contrib.contenttypes',
        'django.contrib.sessions',
...
will enable your project to load Wshop addons.

Installing and configuring addons
---------------------------------

Once ``wshop.addons`` is in your ``INSTALLED_APPS`` list, a section for
...
``entry_points`` section add a ``wshop.addon`` entry point stanza, such as
this (example taken from the `wshop-stripe <https://github.com/wshop/wshop-stripe>`_
addon):
...
.. code-block:: python
    ...
    setuptools.setup(
        # ...
        entry_points={"wshop.addon": "wshop_stripe=wshop_stripe"}
    )
