...
def get_known_settings():
    """
    Get all settings known to Wshop.
...
def validate_templates_configuration():
    ...
    Wshop's admin and default frontend require some Django-Jinja configuration, so
    ...
    for template_engine in django.conf.settings.TEMPLATES:
        ...
        if "DjangoTemplates" in backend:
            raise ImproperlyConfigured(
                ...
                "before Django-Jinja. This configuration will not work correctly with Wshop."
            )
    ...
    raise ImproperlyConfigured(
        ...
        "See the Wshop or Django-Jinja documentation on more information how to configure things correctly."
    )
