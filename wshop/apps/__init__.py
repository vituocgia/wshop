...
"""
Wshop Application API
=====================

Every Wshop Application should define an app config class derived from
`wshop.apps.AppConfig`:class:.
...
To define settings for a Wshop Application, add a ``settings.py`` file
...
See also source code of :obj:`wshop.core.settings`.
...
Applications in :term:`Wshop Base` distribution should use the following
rules for naming their settings.

 1. Each setting should be prefixed with the string `WSHOP_`
 2. Boolean toggle settings should have a verb in imperative mood as
    part of the name, e.g. `WSHOP_ALLOW_ANONYMOUS_ORDERS`,
    `WSHOP_ENABLE_ATTRIBUTES` or `WSHOP_ENABLE_MULTIPLE_SHOPS`.
 3. Setting that is used to locate a replaceable module should have
    ...
    those), e.g. `WSHOP_PRICING_MODULE_SPEC`.
 4. Setting names do NOT have to be prefixed with the application name.
    For example, `WSHOP_BASKET_VIEW_SPEC` which is not prefixed with
    `WSHOP_FRONT` even though it is from `wshop.front` application.
