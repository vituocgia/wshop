...
from wshop.admin.toolbar import Toolbar, URLActionButton
...
from wshop.notify.admin_module import SCRIPT_TEMPLATES_PROVIDE_CATEGORY
from wshop.notify.admin_module.forms import ScriptForm
...
class ScriptEditView(CreateOrUpdateView):
    ...
    def form_valid(self, form):
        ...
        wf = form.save()
        if is_new:
            return redirect("wshop_admin:notify.script.edit-content", pk=wf.pk)
        else:
            add_create_or_change_message(self.request, self.object, is_new=is_new)
            return redirect("wshop_admin:notify.script.edit", pk=wf.pk)
