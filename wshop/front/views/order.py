...
from wshop.front.signals import order_complete_viewed


class OrderCompleteView(DetailView):
    template_name = "wshop/front/order/complete.jinja"
    model = Order
...
class OrderRequiresVerificationView(DetailView):
    template_name = "wshop/front/order/requires_verification.jinja"
    model = Order
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object.user and self.object.user.password == "//IMPLICIT//":
            from wshop.shop.views.activation_views import OneShotActivationForm
