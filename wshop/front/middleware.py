...
from wshop.core.middleware import ExceptionMiddleware
from wshop.core.models import Contact, get_company_contact, get_person_contact
from wshop.core.shop_provider import get_shop
from wshop.front.basket import get_basket
from wshop.front.utils.user import is_admin_user

__all__ = ["ProblemMiddleware", "WshopFrontMiddleware"]

ProblemMiddleware = ExceptionMiddleware  # This class is only an alias for ExceptionMiddleware.


class WshopFrontMiddleware(object):
    """
    Handle Wshop specific tasks for each request and response.

    * Set request attributes that rest of the Wshop front-end rely on.
    ...
    * Make sure that basket is saved before response is returned to the
    ...
    Attributes set for requests:
      ...
      ``request.person`` : :class:`wshop.core.models.Contact`
          :class:`~wshop.core.models.PersonContact` of currently logged
          in user or :class:`~wshop.core.models.AnonymousContact` if
          there is no logged in user.

      ``request.customer`` : :class:`wshop.core.models.Contact`
          Customer contact used when creating Orders.  This can be same
          as ``request.person``, but for example in B2B shops this is
          usually a :class:`~wshop.core.models.CompanyContact` whereas
          ``request.person`` is a
          :class:`~wshop.core.models.PersonContact`.

      ``request.basket`` : :class:`wshop.front.basket.objects.BaseBasket`
    ...
    def _set_shop(self, request):
        """
        Set the shop here again, even if the WshopCore already did it.
    ...
    def _get_maintenance_response(self, request, view_func):
        ...
        if request.shop.maintenance_mode and not is_admin_user(request):
            return HttpResponse(loader.render_to_string("wshop/front/maintenance.jinja", request=request), status=503)


if (
    "django.contrib.auth" in settings.INSTALLED_APPS and
    "wshop.front.middleware.WshopFrontMiddleware" in settings.MIDDLEWARE_CLASSES
):
    user_logged_in.connect(WshopFrontMiddleware.refresh_on_user_change, dispatch_uid="wshop_front_refresh_on_login")
    user_logged_out.connect(WshopFrontMiddleware.refresh_on_logout, dispatch_uid="wshop_front_refresh_on_logout")
