...
#: without having to override a single URL or touch the ``wshop.front`` code.
WSHOP_BASKET_ORDER_CREATOR_SPEC = (
    "wshop.core.basket.order_creator:BasketOrderCreator")

#: Spec string for the Django CBV (or an API-compliant class) for the basket view.
#:
#: This view deals with ``/basket/``.
WSHOP_BASKET_VIEW_SPEC = (
    "wshop.front.views.basket:DefaultBasketView")
...
WSHOP_BASKET_COMMAND_DISPATCHER_SPEC = (
    "wshop.core.basket.command_dispatcher:BasketCommandDispatcher")

#: Spec string for the update method dispatcher used when the basket is updated (usually
#: on the basket page).
WSHOP_BASKET_UPDATE_METHODS_SPEC = (
    "wshop.front.basket.update_methods:BasketUpdateMethods")
...
WSHOP_BASKET_CLASS_SPEC = (
    "wshop.front.basket.objects:BaseBasket")
...
WSHOP_BASKET_STORAGE_CLASS_SPEC = (
    "wshop.front.basket.storage:DatabaseBasketStorage")
...
WSHOP_TEMPLATE_HELPERS_CACHE_DURATION = 60*30
...
#:
#: WSHOP_PERSON_CONTACT_FIELD_PROPERTIES = {
...
#: example which makes the gender field hidden:
#:
#: WSHOP_CHECKOUT_CONFIRM_FORM_PROPERTIES = {
