...
from wshop.core.models import get_person_contact, PersonContact
from wshop.notify.base import Event, Variable
from wshop.notify.script_template.factory import \
    generic_send_email_script_template_factory
from wshop.notify.typology import Boolean, Email, Model, URL


class RegistrationReceived(Event):
    identifier = "registration_received"
    name = _("Registration Received")
    customer = Variable(_("Customer"), type=Model("wshop.Contact"))
...
def send_user_registered_notification(user, request, **kwargs):
    ...
    if activation_key:
        activation_path = reverse('wshop:registration_activate', args=(activation_key,))
