...
from wshop.front.basket import get_basket_order_creator
...
class ConfirmForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        field_properties = settings.WSHOP_CHECKOUT_CONFIRM_FORM_PROPERTIES
...
class ConfirmPhase(CheckoutPhaseViewMixin, FormView):
    ...
    template_name = "wshop/front/checkout/confirm.jinja"
    ...
    def form_valid(self, form):
        ...
        if order.require_verification:
            return redirect("wshop:order_requires_verification", pk=order.pk, key=order.key)
        else:
            return redirect("wshop:order_process_payment", pk=order.pk, key=order.key)
