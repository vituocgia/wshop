...
from wshop.addons.manager import (
...
class AddonEnableDisableForm(forms.Form):
    ...
    def _create_fields(self):
        enabled_addons = get_enabled_addons(settings.WSHOP_ENABLED_ADDONS_FILE)
...
class AddonListView(FormView):
    template_name = "wshop/admin/addons/list.jinja"
    form_class = AddonEnableDisableForm

    def form_valid(self, form):
        old_enabled_addons = get_enabled_addons(settings.WSHOP_ENABLED_ADDONS_FILE)
        ...
        if changes:
            set_enabled_addons(
                settings.WSHOP_ENABLED_ADDONS_FILE,
                new_enabled_addons,
                comment="Written via Wshop admin (user %s; IP %s; time %s)" % (
