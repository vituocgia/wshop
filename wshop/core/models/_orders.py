...
from wshop.core import taxing
from wshop.core.excs import (
...
from wshop.core.signals import (
    payment_created, refund_created, shipment_created,
    shipment_created_and_processed
)
from wshop.utils.analog import define_log_model, LogEntryKind
from wshop.utils.dates import local_now, to_aware
from wshop.utils.money import Money
from wshop.utils.properties import (
...
class OrderStatus(TranslatableModel):
    identifier = InternalIdentifierField(
        ...
        unique=True, help_text=_("Internal identifier for status. This is used to identify the statuses in Wshop."))
...
class OrderStatusManager(object):
    ...
    def ensure_default_statuses(self):
        ...
        will ensure that
        """
        # these values are based on old wshop data
        update_map = {
...
class Order(MoneyPropped, models.Model):
    ...
    def _cache_values(self):
        ...
        if not self.label:
            self.label = settings.WSHOP_DEFAULT_ORDER_LABEL
    ...
    def save(self, *args, **kwargs):
        if not self.creator_id:
            if not settings.WSHOP_ALLOW_ANONYMOUS_ORDERS:
                raise ValidationError(
                    "Anonymous (userless) orders are not allowed "
                    "when WSHOP_ALLOW_ANONYMOUS_ORDERS is not enabled.")
        self._cache_values()
    ...
    def create_payment(self, amount, payment_identifier=None, description=''):
        ...
        :rtype: wshop.core.models.Payment
        """
    ...
    def create_shipment(self, product_quantities, supplier=None, shipment=None):
        ...
        :type product_quantities: dict[wshop.shop.models.Product, decimal.Decimal]
        ...
        :rtype: wshop.core.models.Shipment
        """
    ...
    def create_shipment_of_all_products(self, supplier=None):
        ...
        :return: Saved, complete Shipment object
        :rtype: wshop.shop.models.Shipment
        """
    ...
    def get_known_additional_data(self):
        ...
        for data_dict, name_mapping in (
                (self.payment_data, settings.WSHOP_ORDER_KNOWN_PAYMENT_DATA_KEYS),
                (self.shipping_data, settings.WSHOP_ORDER_KNOWN_SHIPPING_DATA_KEYS),
                (self.extra_data, settings.WSHOP_ORDER_KNOWN_EXTRA_DATA_KEYS),
        ):
    ...
    def get_available_shipping_methods(self):
        ...
        """
        from wshop.core.models import ShippingMethod
    ...
    def get_available_payment_methods(self):
        ...
        """
        from wshop.core.models import PaymentMethod
