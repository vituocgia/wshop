...
def get_unique_reference_number(shop, id):
    ...
    ref_length = configuration.get(shop, ORDER_REFERENCE_NUMBER_LENGTH_FIELD, settings.WSHOP_REFERENCE_NUMBER_LENGTH)
...
def get_running_reference_number(order):
    from wshop import configuration
    from wshop.admin.modules.settings.consts import (ORDER_REFERENCE_NUMBER_PREFIX_FIELD,
                                                     ORDER_REFERENCE_NUMBER_LENGTH_FIELD)
    value = Counter.get_and_increment(CounterType.ORDER_REFERENCE)
    prefix = "%s" % configuration.get(
        order.shop, ORDER_REFERENCE_NUMBER_PREFIX_FIELD, settings.WSHOP_REFERENCE_NUMBER_PREFIX)
    ref_length = configuration.get(
        order.shop, ORDER_REFERENCE_NUMBER_LENGTH_FIELD, settings.WSHOP_REFERENCE_NUMBER_LENGTH)
...
def get_reference_number(order):
    ...
    reference_number_method = configuration.get(
        order.shop, ORDER_REFERENCE_NUMBER_METHOD_FIELD, settings.WSHOP_REFERENCE_NUMBER_METHOD)
