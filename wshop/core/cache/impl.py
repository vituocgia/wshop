...
request_finished.connect(_clear_versions_for_request, dispatch_uid="wshop.core.cache._clear_versions_for_request")
...
def get_cache_duration(cache_key):
    ...
    duration = settings.WSHOP_CACHE_DURATIONS.get(namespace)
    if duration is None:
        duration = DEFAULT_CACHE_DURATIONS.get(namespace, settings.WSHOP_DEFAULT_CACHE_DURATION)
    return duration
