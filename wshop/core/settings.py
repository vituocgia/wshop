...
WSHOP_ORDER_IDENTIFIER_METHOD = "id"
...
#: `wshop.core.pricing.DiscountModule` for details.
WSHOP_DISCOUNT_MODULES = ["catalog_campaigns", "customer_group_discount"]

#: List of identifiers of order source modifier modules.
#:
#: See `wshop.core.order_creator.OrderSourceModifierModule` for details.
WSHOP_ORDER_SOURCE_MODIFIER_MODULES = ["basket_campaigns"]
...
#: other order lines.  See :obj:`wshop.core.taxing` for details.
WSHOP_TAX_MODULE = "default_tax"
...
WSHOP_ENABLE_MULTIPLE_SUPPLIERS = False

#: Indicates whether Wshop should restrict Contact access per Shop
#:
...
#: placed an order.
WSHOP_MANAGE_CONTACTS_PER_SHOP = False
...
#: The order label (see ``WSHOP_ORDER_LABELS``) to apply to orders by default.
#: This should naturally be one of the keys in ``WSHOP_ORDER_LABELS``.
WSHOP_DEFAULT_ORDER_LABEL = "default"
...
WSHOP_ORDER_KNOWN_PAYMENT_DATA_KEYS = []
...
#: in the administration backend.
WSHOP_ORDER_KNOWN_SHIPPING_DATA_KEYS = []
...
#: in the administration backend.
WSHOP_ORDER_KNOWN_EXTRA_DATA_KEYS = []

#: A flag to enable/disable the telemetry system
WSHOP_TELEMETRY_ENABLED = True

#: The host URL for Wshop's telemetry (statistics) system
WSHOP_TELEMETRY_HOST_URL = "https://telemetry.wshop.com"

#: The submission URL for Wshop's telemetry (statistics) system
WSHOP_TELEMETRY_URL = "%s/collect/" % WSHOP_TELEMETRY_HOST_URL

#: The URL to fetch the Wshop installation support id
WSHOP_SUPPORT_ID_URL = "%s/support-id" % WSHOP_TELEMETRY_HOST_URL

#: Default cache duration for various caches in seconds
WSHOP_DEFAULT_CACHE_DURATION = 60 * 30

#: Overrides for default cache durations by key namespace.
#: These override possible defaults in `wshop.core.cache.impl.DEFAULT_CACHE_DURATIONS`.
WSHOP_CACHE_DURATIONS = {}

#: Whether taxes should be calculated automatically in TaxModule
WSHOP_CALCULATE_TAXES_AUTOMATICALLY_IF_POSSIBLE = True

#: Spec which defines an address formatter used to
#: format output string of an Address model instances
WSHOP_ADDRESS_FORMATTER_SPEC = (
    "wshop.core.utils.formatters:DefaultAddressFormatter")

#: Spec which defines an default address model form
WSHOP_ADDRESS_MODEL_FORM = (
    "wshop.core.utils.forms.MutableAddressForm")
...
WSHOP_MAX_DAYS_IN_TELEMETRY = 180
...
WSHOP_AUTO_SHOP_PRODUCT_CATEGORIES = True
...
#:
#: Wshop will iterate over all handlers in order to determinate
#: if some can handle the error. In case of no handler can
#: do the job, a blank response will be returned.
#:
#: A handler must be a subclass of `wshop.core.error_handling.ErrorPageHandler`.
#:
#: If no handler is set (blank), Wshop will use default Django's handlers.
#:
...
WSHOP_SHOP_PRODUCT_SUPPLIERS_STRATEGY = (
    "wshop.core.suppliers.FirstSupplierStrategy")

#: Spec which provides the current shop for a given request and set of parameters.
WSHOP_REQUEST_SHOP_PROVIDER_SPEC = (
    "wshop.core.shop_provider.DefaultShopProvider")
