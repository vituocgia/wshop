...
class Money(numbers.UnittedDecimal):
    ...
    `~wshop.utils.numbers.UnittedDecimal.value` property (preferred way)
    or by casting to `Decimal`.

    Money objects with different currencies cannot be compared or
    calculated with and will raise `~wshop.utils.numbers.UnitMixupError`.
...
def set_precision_provider(precision_provider):
    ...
    `wshop.utils.babel_precision_provider.get_precision`.
