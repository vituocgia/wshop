...
from wshop.importer._mapper import RelatedMapper
from wshop.importer.exceptions import ImporterError
from wshop.importer.importing.meta import ImportMetaBase
from wshop.importer.importing.session import DataImporterRowSession
from wshop.importer.utils import copy_update, fold_mapping_name
...
class DataImporter(object):
    ...
    def manually_match(self, imported_field_name, target_field_name):
        ...
        target_model, wshop_field_name = target_field_name.split(":")
        mapping = self.mapping.get(wshop_field_name)

        mapping["matcher"] = self.matcher
        mapping["setter"] = self.set_extra_match

        self.extra_matches[target_field_name] = imported_field_name
        self.mapping[wshop_field_name] = mapping
        return self.mapping
