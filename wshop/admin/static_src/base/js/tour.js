...
(function($) {
    ...
    $.tour = function(config={}, params) {
        ...
        function getHelpButton(page) {
            let content = "";
            if(page) {
                let helpUrl = "http://wshop-guide.readthedocs.io/en/latest/" + page;
                content += "<br>";
                content += "<p class='text-center'>";
                content += "<a href='" + helpUrl + "' class='btn btn-inverse btn-default', target='_blank'>";
                content += "<i class='fa fa-info-circle'></i> " + gettext("Learn more at the Wshop Help Center");
                content += "</a>";
                content += "</p>";
            }
            return content;
        ...
        if(config.tourKey) {
            tour.on("cancel", () => {
                $.post("/sa/tour/", {"csrfmiddlewaretoken": window.WshopAdminConfig.csrf, "tourKey": config.tourKey});
            });
        }
        tour.start();
        return tour;
