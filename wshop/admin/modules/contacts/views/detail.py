...
from wshop.core.models import CompanyContact, Contact
from wshop.front.apps.registration.signals import company_contact_activated
from wshop.utils.deprecation import RemovedFromWshopWarning
...
class ContactDetailToolbar(Toolbar):
    ...
    def build_renew_password_button(self):
        ...
        if "wshop.front.apps.auth" not in settings.INSTALLED_APPS:
            disable_reason = _("The Wshop frontend is not enabled.")
        elif not self.user:
        ...
        self.append(PostActionButton(
            post_url=reverse("wshop_admin:contact.reset_password", kwargs={"pk": self.contact.pk}),
            name="pk",
    ...
    def build_new_user_button(self):
        ...
        self.append(URLActionButton(
            url=reverse("wshop_admin:user.new") + "?contact_id=%s" % self.contact.pk,
            text=_(u"New User"),
    ...
    def build_new_order_button(self):
        self.append(URLActionButton(
            url=reverse("wshop_admin:order.new") + "?contact_id=%s" % self.contact.pk,
            text=_(u"New Order"),
    ...
    def build_provides_buttons(self):
        ...
        for button in get_provide_objects("admin_contact_toolbar_button"):
            warnings.warn(
                ...
                RemovedFromWshopWarning)
            self.append(button(self.contact))

    def build(self):
        self.append(URLActionButton(
            url=reverse("wshop_admin:contact.edit", kwargs={"pk": self.contact.pk}),
            icon="fa fa-pencil",
            text=_(u"Edit..."),
            extra_css_class="btn-info",
        ))
...
class ContactDetailView(DetailView):
    model = Contact
    template_name = "wshop/admin/contacts/detail.jinja"
