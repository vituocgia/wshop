...
from wshop.front.apps.auth.views import RecoverPasswordForm
...
class UserChangePasswordView(UpdateView):
    ...
    template_name = "wshop/admin/users/change_password.jinja"
...
class UserResetPasswordView(DetailView):
    model = settings.AUTH_USER_MODEL
    template_name = "wshop/admin/users/reset_password.jinja"
    ...
    def process_user(self, user):
        if "wshop.front.apps.auth" not in settings.INSTALLED_APPS:
            raise Problem(_(u"The `wshop.front.apps.auth` app needs to be enabled for password reset."))
