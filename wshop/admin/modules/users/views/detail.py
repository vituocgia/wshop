...
from wshop.admin.toolbar import (
...
from wshop.utils.text import flatten
...
class UserDetailToolbar(Toolbar):
    ...
    def _build_existing_user(self):
        ...
        change_password_button = DropdownItem(
            url=reverse("wshop_admin:user.change-password", kwargs={"pk": user.pk}),
            text=_(u"Change Password"), icon="fa fa-exchange"
        )
        reset_password_button = DropdownItem(
            url=reverse("wshop_admin:user.reset-password", kwargs={"pk": user.pk}),
        ...
        permissions_button = DropdownItem(
            url=reverse("wshop_admin:user.change-permissions", kwargs={"pk": user.pk}),
        ...
        if person_contact:
            contact_url = reverse("wshop_admin:contact.detail", kwargs={"pk": person_contact.pk})
            ...
            contact_url = reverse("wshop_admin:contact.new") + "?type=person&user_id=%s" % user.pk
            menu_items.append(DropdownItem(
                url=contact_url,
                icon="fa fa-plus",
        ...
        if (self.request.user.is_superuser and get_front_url() and
            ...
            self.append(PostActionButton(
                post_url=reverse("wshop_admin:user.login-as", kwargs={"pk": user.pk}),
...
class UserDetailView(CreateOrUpdateView):
    ...
    template_name = "wshop/admin/users/detail.jinja"
    ...
    def save_form(self, form):
        ...
        if getattr(self.object, "is_staff", False) and form.cleaned_data.get("send_confirmation"):
            ...
            admin_url = self.request.build_absolute_uri(reverse("wshop_admin:login"))
            send_mail(
