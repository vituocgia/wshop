    ...
    def _build_order_set_state_button(self):
        ...
        for status in OrderStatus.objects.filter(is_active=True).exclude(pk=self.order.status.pk).order_by("ordering"):
            btn = PostActionDropdownItem(
                post_url=reverse("wshop_admin:order.set-status", kwargs={"pk": self.order.pk}),
                name="status",
                value=status.pk,
                text=status.name,
            )
...
class CreatePaymentAction(DropdownItem):
    def __init__(self, object, **kwargs):
        kwargs["url"] = reverse("wshop_admin:order.create-payment", kwargs={"pk": object.pk})
...
class SetPaidAction(PostActionDropdownItem):
    def __init__(self, object, **kwargs):
        kwargs["post_url"] = reverse("wshop_admin:order.set-paid", kwargs={"pk": object.pk})
...
class CreateShipmentAction(DropdownItem):
    def __init__(self, object, **kwargs):
        kwargs["url"] = reverse("wshop_admin:order.create-shipment", kwargs={"pk": object.pk})
...
class CreateRefundAction(DropdownItem):
    def __init__(self, object, **kwargs):
        kwargs["url"] = reverse("wshop_admin:order.create-refund", kwargs={"pk": object.pk})
...
class EditAddresses(DropdownItem):
    def __init__(self, object, **kwargs):
        kwargs["url"] = reverse("wshop_admin:order.edit-addresses", kwargs={"pk": object.pk})
