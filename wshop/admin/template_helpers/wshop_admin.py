...
"""
This module is installed as the `wshop_admin` template function namespace.
"""
...
from wshop.admin import menu
from wshop.admin.breadcrumbs import Breadcrumbs
from wshop.admin.utils.urls import (
    get_model_url, manipulate_query_string, NoModelUrl
)
from wshop.core.models import Shop
from wshop.core.telemetry import is_telemetry_enabled
...
def get_support_id(context):
    ...
    if is_telemetry_enabled():
        support_id = configuration.get(None, "wshop_support_id")
    return support_id


# TODO: Figure out a more extensible way to deal with this
BROWSER_URL_NAMES = {
    "select": "wshop_admin:select",
    "media": "wshop_admin:media.browse",
    "product": "wshop_admin:shop_product.list",
    "contact": "wshop_admin:contact.list",
    "setLanguage": "wshop_admin:set-language"
}
...
def get_config(context):
    ...
    qs = {"context": url_name}
    return {
        "searchUrl": manipulate_query_string(reverse("wshop_admin:search"), **qs),
        "menuUrl": manipulate_query_string(reverse("wshop_admin:menu"), **qs),
...
def is_multishop_enabled(context):
    return settings.WSHOP_ENABLE_MULTIPLE_SHOPS is True
