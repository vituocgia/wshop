...
from wshop.xtheme._theme import get_middleware_current_theme
from wshop.xtheme.editing import add_edit_resources
...
class XthemeEnvironment(Environment):
    ...
    Enable by adding ``"environment": "wshop.xtheme.engine.XthemeEnvironment"``
    ...
    def get_template(self, name, parent=None, globals=None):
        ...
        :rtype: wshop.xtheme.engine.XthemeTemplate
        """
    ...
    def _get_themed_template_names(self, name):
        ...
        `wshop/front/bar.jinja`, we'll look at `mystery/wshop/front/bar.jinja`, finally at `wshop/front/bar.jinja`.

        Mystery theme also can define default template dir let's say `pony`. In this scenario we're looking up
        `wshop/front/bar.jinja` from `mystery/wshop/front/bar.jinja` then at `pony/wshop/front/bar.jinja` and
        finally at the default `wshop/front/bar.jinja`.
        ...
        :rtype: str|list[str]
        """
        if name.startswith("wshop/admin"):  # Ignore the admin.
            return name
